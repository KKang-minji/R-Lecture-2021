...
arrange(europe_pop, desc(lifeExp)) #내림차순(높은것부터)
...
```{r}
#2007년 아프리카 국가별 평균 인구 수
summarize(africa_pop, pop_avg=mean(pop))   
#continent별 평균 인구 수
summarize(group_by(gapminder, continent), pop_avg=mean(pop))
#국가별 평균 수명
summarize(group_by(gapminder, country), lifeE_avg=mean(lifeExp))
