...
tmp <- subset(data, gender==1)      #gender가 1인것만 가지고 옴
weight <- tmp[[3]]                  #몸무게만 가져옴

barx <- mean(weight)            #여자 신생아 몸무게 평균
s <- sd(weight)                 #여자 신생아 몸무게 표준편차
n <- length(weight)             #돗수
h0 <- 2800                      #모집단의 평균(기존의 여아 평균 몸무게 = 2800)
t.t <- (barx - h0) / (s / sqrt(n))             #검정통계량

alpha <- 0.05                   #95퍼센트 신뢰성
c.u <- qt(1-alpha, df=n-1)      #누적확률이 0.95가 되는 위치  #df:자유도
p.value <- 1 - pt(t.t, df=n-1)  #검정통계량 2.23보다 클 확률

#R에서 제공하는 t-test
t.test(weight, mu=2800, alternative="greater")    #alternative:대립가설
  #p-value = 0.01963 0.05보다 작기때문엔 대립가서 채택
```
